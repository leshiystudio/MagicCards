...
**Версия:** 0.14 · **Дата:** 16.09.2026 · **Статус:** спецификация правил и план развития. Фактическая реализация и её ограничения описаны в [статусе проекта](../docs/IMPLEMENTATION.md).
...
- Бой сохраняет атаки игрока и назначение блоков; в текущем профиле отдельный компонент attack_target разрешает атаковать, например, носителя Силы. Базовое существо его не имеет. Свойства вроде полёта ограничивают блокирование, а не вводят прямую атаку на существо.
...
**Рабочие допущения:** TypeScript и браузерный стек; ограниченная модель блоков; числа баланса, размеры колод/руки, правила производства/хранения и стартовые карты ниже. Это предложения для старта, а не ранее утверждённые решения. Их изменения оформляются следующей редакцией.
...
| R-08 | Конец | Здоровье игрока ≤ 0 означает поражение при проверке состояния; обоих одновременно — ничью. После результата стек не разрешается. Concede доступна любому участнику в любом стабильном незавершённом состоянии, включая чужой приоритет или PendingDecision; она не прерывает атомарно исполняемый frame. |
...
CharacterRef и селекторы *_character означают только «игрок или существо». В редакторе эта область так и подписывается; слово «персонаж» не используется как общее название цели. Тип types:champion («Носитель Силы») — отдельный тип, не часть этого объединения. Колода не содержит ResourceDefinition, AbilityDefinition, EffectTokenDefinition или InteractionRule как игровых карт: карта с typeId: types:resource ссылается на типы ресурса через производство. Произвольный порядок допустимых карт задаёт исходный список для перемешивания, а библиотечные ссылки разрешаются до матча. Ограничение 24/2 с явными исключениями для источников — рабочее правило стартового профиля, проверяемое и редактором, и ядром.
...
**Подтверждено владельцем:** оба режима входят в первое ядро; режим по умолчанию — ресурсы из карт колоды. Начальный набор содержит Свет и Тьму и допускает ручное дополнение новыми ресурсами/картами. Названия режимов описывают нашу экономику, а не полное копирование Hearthstone/MTG.
...
|---|---|---|
| READY | Сбросить sourcePlaysUsed; применить экономику 3.2/7.20 и owner_turn_start-очистку; развернуть все свои permanent с компонентом exhaustion, включая артефакты, чары и источники. В deck_sources автоматического прироста нет. | Без приоритета перейти к UPKEEP. |
...
| CLEANUP | Одновременно снять markedDamage со всех permanent с health и убрать модификаторы и временные непрерывные эффекты «до конца хода». | Проверка состояния и передача хода. |
...
При заполненной требуемой slot-группе постоянную карту нельзя объявить. Если место закончилось уже после объявления, при разрешении карта уходит на кладбище без входа и смерти на поле. Для существа используется unit-slot; воскрешение оставляет карту на кладбище по 7.24. Отменённое в стеке существо также не вызывает on_death.
...
**R-15. Атака.** Активный игрок выбирает развёрнутых носителей combat_attacker с effective attack > 0 и выполненной готовностью exhaustion. Базовые существа готовы атаковать со следующего собственного хода. Для каждого атакующего указывается защищающийся игрок либо его постоянный объект с attack_target (7.21); базовые существа сами целью атаки не являются. Объявление поворачивает атакующих, кроме vigilance.
...
**R-22. Пределы.** Единица ограничения AST — один behavior (triggered/activated/continuous/on_resolve/interaction_rule/resource_activated) после связывания всех ref; inline-способность — один behavior, корневая программа effects карты — один неявный on_resolve. На единицу до 256 семантических узлов и глубины 16; на AbilityDefinition до 32 behaviors и суммарно 4096 узлов. Подробный счёт — 16.8. Произвольного кода и циклических ссылок разворачивания одного AST нет; ссылки контента на шаблоны фишек допускают игровые циклы только через очередь по 7.18. До 1000 исполненных операций на внешнюю команду. Дополнительно — до 1000 последовательных разрешений StackItem без нового розыгрыша или смены шага; PassPriority этот счётчик не сбрасывает. Превышение даёт техническую остановку с диагностикой, а не победу или игровую ничью.
...
| on_enter | Собственный вход любого permanent; канонический алиас permanent_entered с permanent = source | PermanentSnapshot; для соответствующих типов доступна проекция creature/resource |
...
| resource_entered / resource_played / resource_left | Вход/выкладывание/уход карты-источника | ResourceCardRef/ResourceCardSnapshot, игрок и причина; on_enter — канонический permanent_entered с resource-проекцией, 7.20 |
...
On_enter связывается с собственным permanent_entered; creature/resource payload доступен как типизированная проекция того же transitionId. Одна подписка на алиас срабатывает один раз, даже если у карты несколько категорий. On_death остаётся собственным creature_died и требует у носителя категорию creature. При её отсутствии привязка отклоняется валидатором; нет молчаливой неработающей способности. Общая библиотека должна ограничить typeFilter этой категорией. Для реакции не-существа на свой уход используется permanent_left с условием permanent = source. Две явно разные способности могут независимо подписаться на общий и специальный факт. Подробности проекций — 7.24.
...
**Пример: каждый успешный собственный добор восстанавливает 1 здоровье игроку.** Это inline-способность носителя на поле; библиотечная версия может применяться к creature/player с соответствующими activeZones/activeOnPlayer.
...
Определение карты содержит id, name, typeId, ResourceCost и совместимые с CompiledCardType поля: характеристики, способности и attachTo. Скорость и путь розыгрыша задаёт тип. Базовый types:resource не имеет обычной цены и боевых характеристик; составной источник может получить их компоненты, но способ source_deploy не оплачивает cost. Экземпляр — конкретная карта или фишка с instanceId/generation, зоной и состоянием своих компонентов. Изменение экземпляра не меняет определение или другие копии; TokenDefinition/isToken описаны в 7.18.
...
| CharacterRef | Игрок или существо | Область *_character; damage также принимает более общий DamageableRef |
...
В v3 словарь также содержит player (любой из двух игроков), friendly_permanent/enemy_permanent/all_permanents, enemy_damageable и graveyard_card/graveyard_creature_card по 7.21–7.24. Player — одна выбираемая цель PlayerRef, не коллекция и не владелец source по умолчанию.

Составные фильтры входят в MVP ядра. Filter — Boolean-выражение по candidate и контексту (локальная переменная item в foreach задаётся отдельно); оно использует сравнения/and/or/not из 7.4. Например, «вражеские существа с атакой меньше атаки источника» — обычный запрос с less и read_property. Для разовых эффектов доступен effective, для непрерывных селекторов аур разрешён только BaseView по 7.7. Дополнительные селекторы: all_friendly_creatures, other_friendly_creatures, all_creatures и character (игрок или существо любой стороны). Источник исключается по instanceId/generation, а не по cardId.
...
Логические операции: equal, not_equal, less, less_or_equal, greater, greater_or_equal, and, or, not. And/or вычисляются слева направо с коротким замыканием. If вычисляет только выбранную ветвь. Equal/not_equal сравнивают одинаковые скалярные типы. Ссылочные подтипы приводятся к общему семейству идентичности: CreatureRef, CombatantRef, PermanentRef, CardRef и их capability-подтипы сравниваются по instanceId + generation; PlayerRef — по playerId, StackItemRef — по itemId. Поэтому attacker: CombatantRef можно сравнить с source: CreatureRef. Для объединения PlayerRef | CardRef сначала сравнивается discriminator семейства, разные варианты не равны. Snapshot не приводится к Ref неявно; Optional требует проверки/разворачивания.
...
| filter | Collection<T> | Узел над коллекцией: items, as, where; сохраняет исходный порядок подходящих элементов |
...
#### Две разные роли слова filter

Поле selector.filter — Boolean-выражение над context.candidate, не самостоятельная операция обхода. Узел {type: filter, items, as, where} — выражение Collection<T> над уже полученной коллекцией; where читает именованный item. Их схемы различны и однозначно определяются положением в AST. Candidate существует только в поле фильтра селектора, item — только в лексической области filter/sum/foreach. Они не являются альтернативными неявными именами одного объекта. Вложенный селектор создаёт свой candidate и не меняет внешний item.

Предикат is_damaged имеет единственное каноническое поле object: HealthRef. В фильтре явно указывается {context: candidate}, в условии эффекта — выбранная цель. Поле object обязательно и в фильтре, и вне него; target вместо object и неявный candidate не поддерживаются. Is_damaged означает здоровье ниже максимального (для permanent — markedDamage > 0); не проверяет броню или Силу.
...
Foreach входит в ядро: при входе фиксируется конечная коллекция; каждый элемент обрабатывается один раз в порядке instanceId/порядке исходного события. Перемещение карты не добавляет её повторно в обход. Не более 128 элементов одной коллекции обхода; превышение — диагностическая ошибка. Foreach, filter и sum связывают элемент через as: name и читают его через {type: item, name}; имя видимо только в теле/where соответствующего узла. Вложенные области не переопределяют активное имя неявно.
...
| modify_keyword | target: совместимый PermanentRef с ability_host или множество; operation: grant/remove; keyword; duration | Разовый вклад для любого объявленного keyword; remove доминирует над grant, правила 7.16 |
...
| grant_ability | target: совместимый AbilityHostRef либо игрок/множество; ability: ref/params либо inline; duration | Создать AbilityInstance, проверить hostRequirements и stacking; ограничения выдаваемых continuous по 7.17 |
...
Поддерживаются четыре формы: effects самой разыгранной карты, activated, triggered и continuous. У записи способности явно указан kind; один trigger без kind не определяет её вид.
...
```json
{
  ...
  "attack": 2,
  "health": 4,
  "text": "Две активации: 2 ресурса и поворот — 2 урона врагу; 2 жизни и жертва другого своего существа — лечение своему игроку или существу на атаку жертвы.",
  "keywords": [],
  "abilities": [
    {
      "id": "charged_strike",
...
| sacrifice | Выбранные свои PermanentRef на поле; селектор, фильтры и количество соблюдены | Общий permanent_left; DeathRecord(reason = sacrifice) только для категории creature, resource_left при source_deploy; снимки до оплаты |
...
Жертва любого permanent создаёт permanent_left. Категория creature дополнительно даёт creature_died/on_death, source_deploy — resource_left; составной источник-существо даёт обе проекции одного перехода, 7.20/7.24. Эти триггеры лежат выше оплаченной способности; после пары пасов первыми разрешаются они. Сама способность уже существует и сохраняется после жертвы источника.
...
- grant_ability: inline triggered/activated либо ссылка на библиотечную AbilityDefinition. В текущем профиле выдаваемое определение может также иметь привязанные interaction_rule и continuous со stat/rule-вкладами; повторная непрерывная выдача/подавление способностей или keyword внутри полученного определения запрещена по 7.17.
...
В первом ядре применяется фиксированный конвейер. Библиотечные определения и источники активности нормализуются по 7.17; порядок следующий:
...
6. **EffectiveView:** attack = max(0, базовая атака + все итоговые вклады). Это значение возвращают read_property(read: effective), события и effective-снимки; отдельного отрицательного rawAttack в DSL нет. MaxHealth может стать ≤ 0. Текущее здоровье = maxHealth − markedDamage.
...
### 7.10. Единый формат, наборы и воспроизведение

До первой реализации существует **один актуальный формат**. Предыдущие редакции этой заметки — история проектирования, не форматы клиентов, которые нужно поддержать. Адаптеров прошлых редакций, таблицы соответствий старых типов и переключателя режима совместимости в первом ядре нет. Все нормативные примеры, fixtures и экспорты используют текущие поля.

Различать schemaVersion (структура данных), rulesetVersion (правила), packVersion (контент/баланс) и engineVersion (реализация). Текущие проектные значения: **schemaVersion = 3, rulesetVersion = 0.12**. Число 3 — идентификатор текущей схемы, не требование реализовать схемы 1 и 2. Совместимость будущих выпусков будет определяться после появления реальных сохранений/публикаций.

CardDefinition использует typeId, ResourceCost — объект resources/generic, AbilityDefinition — hostRequirements и activeZones/activeOnPlayer, DurationSpec — объект с type. Связывание inline-способностей, числовых/Boolean-литералов и keywords во внутреннее представление — обычная компиляция текущего языка, не преобразование старого формата. Неизвестные поля и неподдерживаемые версии отклоняются с адресной ошибкой.

**Манифест небольшого учебного набора:**
...
Requires должен покрывать все используемые расширяемые конструкции по замыканию AST/refs. Неизвестная/неподдерживаемая возможность и пропуск нужной — ошибка; известный лишний пункт допустим с предупреждением, но эталонные манифесты содержат минимальный список. Базовые компоненты, зоны, обычная generic-цена, литералы/ссылки и базовые Boolean-операции входят в ruleset.

Требование события добавляется за **явную подписку контента** на него, в том числе через on_enter/on_death; испускание события штатной операцией не требует отдельного event.* в наборе. Поэтому производство ресурса требует ability.resource_activated/content.resources, а event.resources требуется только подписке на resource_* и не нужно duality. Семантика публикации событий всё равно обязательна для реализации операции.

Behavior.interaction_rule включает связывание шаблона, проверку ролей и вычисление привязанного InteractionRule. Отдельной дублирующей capability для тех же правил нет; базовый механизм globalInteractionRules принадлежит ruleset. Возможности предикатов привязанного правила учитываются через ref.

RulesetManifest содержит id/version/schemaVersion, requires, встроенные типы/категории, creatureCategoryId, AbilityDefinition/InteractionRule, EventDefinition и globalInteractionRules. CardPackManifest может содержать subtypes/tags/colors, cardTypes/typeCategories, resourceDefinitions, abilityDefinitions, tokenDefinitions, effectTokenDefinitions, interactionRules и dependencies. Необязательные списки пусты; загрузка шаблона не делает правило глобальным.

ContentSnapshot хранит исходные и проверенные определения, зависимости, версии и хеши. Изменение файла карты не меняет начатую или сохранённую партию. Replay использует свой полный снимок и указанную поддерживаемую семантику; неподдерживаемая версия отклоняется, автоматического пересчёта по новым правилам нет. Это контракт будущих сохранений, а не свидетельство уже существующих записей старого движка.

Новая комбинация известных компонентов меняет packVersion; новая семантика требует кода/схемы/rulesetVersion и проверок. Например, отсутствие effect.return_to_hand означает отказ загрузки требующего её набора, а не пропуск эффекта.
...
Редактор использует метаданные реестра: damage показывает цель и величину; draw — игрока и количество; if — условие и две ветви. Проверяются types, hostRequirements/activeZones/activeOnPlayer, параметры и доступный контекст. Ошибка показывается рядом с полем, а сохранение/запуск проходят тот же валидатор, что загрузка набора.
...
Каждая смерть создаёт DeathRecord: eventId, entityRef(instanceId, generation), definitionId, owner, controller, isToken, creatureSnapshot (effective-характеристики, subtypes/tags/colors/keywords и остальные поля 6.2 перед уходом), fromZone, toZone, reason, frameId, combatId при наличии и ссылки на связанные события урона. Reason и reasons используют общий закрытый список причин ухода: destroy, sacrifice, lethal_damage, zero_max_health, zero_power, invalid_attachment. Reason — ведущая причина, reasons — непустой список всех оснований этой операции; для явного destroy/sacrifice он одноэлементный. CheckpointId и stateCheckWave отдельно отмечают проверку состояния и номер волны (с 1); для явного перемещения вне checkpoint оба null. Aura_cascade не является причиной: каскад выражается stateCheckWave > 1 и causeId/frameId. Для DeathRecord по-прежнему требуется категория creature и переход battlefield → graveyard; составное существо с power/attachment может умереть по соответствующей причине.
...
Если атакующих не объявили, бой всё равно завершается с пустым attackers; смерти от заклинаний в COMBAT_BEGIN могут быть в deaths. Способность «после того, как вы атаковали» дополнительно требует count(event.attackers) > 0 и event.player = controller.
...
CombatDamageDeaths — существа, погибшие в первой волне проверки состояния сразу после COMBAT_DAMAGE из-за markedDamage ≥ положительного maxHealth и получившие положительный урон в этом боевом пакете. Смерти следующих волн от снятия ауры попадают только в deaths с фактической reason = lethal_damage либо zero_max_health и stateCheckWave > 1. Существа с maxHealth ≤ 0 считаются zero_max_health. Это проверяемое определение собственного профиля; оно не пытается угадывать единственную причинную карту.
...
Пассивная способность постоянного источника после **вашего** боя с хотя бы одним атакующим наносит вражескому игроку N урона и лечит выбранного своего игрока или существо на N. N равно всем смертям за бой, включая ответы, жертвы и каскады аур.
...
Create_delayed_trigger для combat_ended требует незакрытый CombatScope. Если текущего combatId нет (например, MAIN_1/MAIN_2 или уже закрытый COMBAT_END), операция штатно ничего не создаёт; это не technical_error и не подписка на будущий бой. Для вызова внутри combat_ended можно использовать snapshot event.combatId только как историю, но повторно открыть этот scope нельзя. Внутри активного боя подписка связывается с единственным текущим combatId и удаляется после его закрытия. Условия автора могут заранее проверить подходящий контекст; выбор посреди effects не возникает.
...
|---|---|---|
| typeId | Ссылка на определение типа с компонентами | types:creature, types:spell, types:resource |
| categories | Категории собранного типа для правил и фильтров | categories:creature, categories:artifact |
...
Сначала selector определяет широкое множество по типу/зоне/стороне. Затем filter сужает его. Без filter селектор возвращает всё множество, определённое его типом, зоной и стороной. Один и тот же Predicate AST используется для объявляемых targets и автоматических множеств.
...
| is_damaged | обязательный object: HealthRef | Повреждения permanent или здоровье игрока ниже максимума |
...
Все библиотечные определения используют hostRequirements, activeZones и activeOnPlayer по 7.21. Допустимость носителя определяется компонентами и явным typeFilter, а не закрытым списком названий типов. Примеры ниже предназначены для реализации одного актуального формата.
...
Короткий фрагмент иллюстрирует ссылку, но не является полным определением: interaction_rule требует **id и hostRole**, как в полном примере ниже. Недостающая запись не допускается валидатором.
...
Flight/recovery — локальные id прикреплений внутри карты; ref — глобальные id определений. Они решают разные задачи: несколько прикреплений одного определения должны различаться. У каждого прикрепления в CardDefinition.abilities обязателен локальный id — как для ссылки, так и для inline-способности. Временная выдача через grant_ability получает origin от ядра по контракту ниже.
...
В текущем профиле hostRequirements требует постоянного носителя с combat_body и ability_host; activeZones ограничивает работу полем. Обычное одноразовое заклинание не удовлетворяет требованиям; карта бойца в руке или стеке не участвует в ограничениях блокирования. Stacking: unique означает один действующий набор поведения Полёта независимо от числа источников выдачи. KeywordId сохраняет возможность спросить has_keyword(flying) и использовать форму keywords.
...
В текущем профиле примеры flying/spectral/contact_spectral из 7.15 являются шаблонами привязанных правил: flying использует hostRole attacker; spectral — attacker для block и target для contact. Они исключены из глобального списка. Все применимые ограничения продолжают соединяться через AND; hostRole сужает область, а не отменяет чужое require.

#### Допустимый носитель, зона и цель эффекта

| Носитель | Требования | Область активности |
|---|---|---|
| Постоянная карта | permanent + ability_host и требования каждого behavior | activeZones: [battlefield] |
| Одноразовое заклинание | one_shot + ability_host | activeZones: [stack]; on_resolve при разрешении карты |
| Игрок | Подходящие health/ability_host и совместимое тело | activeOnPlayer: true, состояние in_game |

Source — носитель способности, grantSource/originGrant — источник её выдачи. Controller для игрока — сам игрок, для карты определяется общими правилами. Snapshot фиксируется при событии/создании StackItem.

HostRequirements проверяются по телу каждого behavior и всем разрешённым носителям. Декларация не позволяет читать attack у объекта без attack или платить tap без exhaustion. Активность в зоне и совместимость устройства — разные проверки: карта существа на стеке сохраняет свой тип, но её battlefield-подписки там не работают.

**Носитель и получатель эффекта — разные роли.** Полёт нельзя прикрепить к spell. Однако spell может иметь эффект grant_ability, выбирающий creature и выдающий ему abilities:flying. В этом случае проверка hostRequirements относится к существу-получателю. Неправильное прикрепление в JSON отклоняется до матча; динамически исчезнувший/неподходящий получатель обрабатывается по R-14 и контракту эффекта.

PlayerDefinition/config может содержать abilities тем же форматом ссылок. У игрока нет generation карточной зоны: AbilityInstance.host хранит PlayerRef. Временная выдача игроку допускается, если выполняется hostRequirements; permanent_on_instance для игрока означает до конца матча. Новых героев, портретных карт или дополнительных зон это само по себе не вводит.

#### Поведение описывается конструкциями DSL
...
Область continuous-behavior задаётся activeZones/activeOnPlayer определения. Inline-способность по умолчанию получает область своего типа; явный activeZones может её сузить, но не разрешает поведение в неподдерживаемой зоне. Singular activeZone не входит в схему. Self для игрока внутри такой непрерывной программы означает игрока, но add_stats применяется к PermanentRef с соответствующим attack/health-компонентом; эта операция не меняет максимум жизни PlayerRef. ActiveOnPlayer: true для такого тела незаконен. Лечение игрока поддерживается через обычный heal.
...
Она прибавляет 2 к maxHealth через ModifierContribution, не меняя CardDefinition.health и markedDamage. Снятие этого экземпляра способности удаляет его health_bonus; checkpoint может обнаружить смерть по R-19. Две выдачи от разных источников дадут +4, поскольку stacking = per_source.
...
Разовое действие **effect.grant_ability** использует тот же план выдачи, что operation: grant_ability внутри continuous. В эффекте target — совместимый AbilityHostRef/PlayerRef или разрешённое множество, ability — ссылка с params либо inline-способность, duration — одна из 16.7. Локальный id внутри ability не нужен при временной выдаче: уникальный origin создаёт ядро.
...
|---|---|---|
| unique | Все источники хранятся, но для одного host/definitionId действует один набор behaviors; параметры у unique запрещены в текущем профиле | Удалить один источник; пока остаётся другой, способность продолжает работать |
...
KeywordId необязателен. Если он указан, definition обязано иметь stacking unique, пустые parameters и проверяемые hostRequirements для всех своих behaviors; typeFilter при необходимости дополнительно ограничивает категории носителя. Это сохраняет булеву семантику keywords. Не всякая библиотечная способность обязана быть keyword: Стойкость и Регенерация выше работают без него.

Grant_keyword/remove_keyword и modify_keyword — текущие операции выдачи/маскирования соответствующей уникальной AbilityDefinition. При remove_keyword(flying) все текущие источники Полёта у получателя остаются записаны, но способность не действует; новые выдачи тоже не обходят маску по 7.16. После истечения маски восстанавливается одно эффективное поведение от оставшихся источников. Ref-вариант выдачи не может обойти этот запрет.
...
- Маски suppress_ability в текущем профиле — разовые эффекты; continuous-модификатор, подавляющий другую произвольную способность, не вводится. Remove_keyword у ауры остаётся допустимым, поскольку keyword-definition не может сама выдавать/снимать keyword.
...
Представление конструкций текущего языка:

| Форма текущего языка | Внутреннее представление |
|---|---|
| keywords: [flying] | Intrinsic-ссылка на встроенную abilities:flying с детерминированным локальным id |
| Inline activated/triggered/continuous в abilities | Локальное AbilityDefinition с одним behavior; локальный id используется для команды |
...
**Уточнение владельца:** нужны обе механики. Аура постоянно влияет на подходящих существ, пока действует её источник. Отдельная срабатывающая способность при входе существа один раз выдаёт выбранному множеству существ сохраняющиеся эффекты. Эти эффекты могут быть усилениями, ослаблениями или предоставлять другое поведение; они не сводятся к разовому присваиванию attack/health.
...
EffectTokenDefinition хранится в библиотеке набора. Канонические поля: id, name, hostRequirements, components. В текущем профиле носителем может быть любой постоянный объект с effect_token_host и совместимыми компонентами, в том числе артефакт, чары или источник. Ограничение конкретной категорией при необходимости задаётся через typeFilter. Одноразовая карта вне battlefield и игрок не получают этот жизненный цикл автоматически. Пустой components разрешён для видимого маркера, который сами правила могут посчитать. Определение само по себе ничего не выдаёт: нужна операция apply_effect_token.
...
Read_effect_token_count — чистое Integer-выражение с object: TokenHostRef/PermanentSnapshot (включая CreatureRef/CreatureSnapshot), definitionId и read: effective/snapshot. Для живого CreatureRef effective означает количество фактически прикреплённых жетонов, независимо от маски предоставляемой ими способности; исчезнувшая incarnation даёт 0. Для CreatureSnapshot read: snapshot читает сохранённые counts. Несовместимые режимы/типы отклоняются. Это чтение предназначено для condition/effects и снимков; в BaseView-селекторах/величинах continuous оно запрещено, как другие effective-зависимости 7.17.
...
Жетоны эффектов входят в E8, а формы/индикаторы — в W2/W1 для P1. Modify_stat/modify_keyword/grant_ability создают собственные вклады или экземпляры способностей. Они не создают EffectTokenInstance и не учитываются read_effect_token_count; для жетона требуется явный apply_effect_token. Для нового явно именованного усиления/ослабления, которое нужно показывать, считать или снимать целиком, автор выбирает EffectTokenDefinition.
...
CardDefinition.cost — объект ResourceCost, а spend_resource указывает resources и/или generic непосредственно в записи платежа; amount у spend_resource не поддерживается. Для цены «любые N единиц» используется generic: N. Переключение экономики не меняет типизированную цену: light/dark не превращаются в energy. В automatic_energy_v1 generic оплачивается выбранным core:energy.
...
#### Карта-источник и её выкладывание

Базовая карта с typeId: types:resource — физический постоянный источник на battlefield. Определение содержит id/name/typeId/abilities; экземпляр хранит owner/controller, incarnation, tapped, enteredTurn/enteredSequence. У базового типа нет attack/health, боевых keywords или обычного cost розыгрыша; путь розыгрыша задаётся в CardTypeDefinition.play. Дополнительные расходы активаций задаются в abilities. Карта приходит в руку обычным добором и учитывается в лимите руки/размере колоды. Составной источник с дополнительными компонентами подчиняется их требованиям по 7.21.
...
Счётчик успешных выкладываний сбрасывается в собственный READY. Неверная команда ничего не расходует. Базовый types:resource имеет exhaustion.readyDelay = none и может производить ресурс в ход входа; у составного источника готовность определяется его компонентом exhaustion. В READY разворачиваются все свои permanent с exhaustion, независимо от категорий. Базовый types:resource не имеет боевого тела/unit-слота/damageable и не входит в creature/character; составной тип может иметь эти компоненты и категории и тогда подчиняется их правилам. Технический предел 128 source_deploy-объектов на игрока дополнительно к общему пределу permanent — защита исполнения, не балансное правило.
...
GameEvent по-прежнему содержит eventId/frameId/causeId. При нескольких типах ресурса события изменения пула публикуются в лексикографическом порядке resourceId после атомарного изменения вектора. Resource_played идёт после resource_entered; это разные факты. Канонический on_enter любого permanent — собственный permanent_entered; creature/resource payload предоставляются типизированными проекциями, а не отдельными подписками алиаса. On_death допустим только при доказанной категории creature: у обычного types:resource/артефакта/чар/носителя Силы отклоняется при загрузке, у составного источника-существа законен. Для общего ухода используется permanent_left, для source_deploy — также resource_left.
...
CardPackManifest получает resourceDefinitions. Пресеты могут храниться отдельными JSON с GameConfig и id рекомендуемых колод. Валидатор проверяет definitions/refs, poolPolicy, совместимость стоимости/производства с выбранным режимом, typeId/метод source_deploy, исключения копий, ресурсные способности и источник оплаты. Набор-библиотека может содержать определения для разных режимов; проверка активных ресурсов применяется к выбранной колоде и полному замыканию реально используемых способностей/шаблонов, а не запрещает хранить другую библиотеку рядом.

Capabilities: content.resources, economy.deck_sources, economy.automatic_growth, ability.resource_activated, effect.gain_resource, cost.typed_resources, expression.read_resource, event.resources. Режимы поддерживаются одной реализацией пула/платежей/способностей; различаются политикой READY и разрешением PlayResourceCard. Пользователь не добавляет новый режим произвольной строкой: новые типы ресурсов — данные, новые экономические правила — версия ядра.
...
Для canAttack/canBlock редактор добавляет/показывает зависимости, а загрузчик проверяет их независимо от UI. Тип с canAttack: true без health/attack/damageable/exhaustion/slot отклоняется. Артефакту без этих компонентов нельзя незаметно приписать 0/0 и затем уничтожить проверкой существ.
...
[
  ...
  },
  {
    ...
    "components": {
      ...
      "attack": {},
      "exhaustion": {
        "readyDelay": "controller_turn_start"
      },
      "damageable": {
        "sink": "health"
      },
...
Общий battle-unit slot требуется всем атакующим/блокирующим независимо от категории; нельзя обойти вместимость поля, назвав бойца артефактом. Поле настройки maxCreaturesPerPlayer сохраняется как совместимый псевдоним предела unit-слотов. Артефакт без боя и Носитель Силы без атаки/блока его не занимают; общий технический предел постоянных объектов — 256 на игрока.
...
В DeclareAttackers каждая запись содержит attacker и defender: PlayerRef либо вражеский PermanentRef с attack_target. Defender — необязательное поле команды с явно определённым значением по умолчанию: противник-игрок. Базовые существа не получают canBeAttacked; бой с ними всё ещё через блоки. Types:champion допускает атаку на себя — это явная настройка нового типа, а не следствие имени «персонаж».

Защищающийся игрок назначает блоки по правилам 1:1 для атак на себя и свои объекты. Незаблокированный удар идёт в сохранённого defender, заблокированный — в блокирующего. Если defender покидает поле до урона, соответствующие атакующие/их блокирующие удаляются из этого боя без перенаправления урона игроку; уже произошедшие события объявления остаются. Keyword-досягаемость по-прежнему ограничивает блоки. Изменение здоровья/Силы цели проходит через её damageable.sink.

#### Загрузка и масштабирование

CardPackManifest содержит cardTypes и typeCategories, полное замыкание extends/refs входит в ContentSnapshot. CardDefinition указывает typeId; поля характеристик проверяются по собранным компонентам. Имена категорий не выбирают обработчик и не подмешивают другой тип. Один актуальный формат описан в 7.10; версионных преобразований в первом ядре нет.

Один ComponentRegistry проверяет типы, строит поля редактора и связывает обработчики. Игровые системы выбирают объекты по capability/zone, а именованные запросы — по категориям профиля. UI/бот не содержат независимого списка «какие имена типов умеют атаковать». Новый тип на существующих компонентах меняет packVersion; новый алгоритм требует кода, capability и rulesetVersion.

Затраты — валидатор комбинаций, снимки, общие цели, bindings и сценарии взаимодействия. Эта работа выполняется до первого кода без слоёв совместимости с прошлыми черновиками. Полноценная ECS-библиотека, пользовательский JavaScript и множественное наследование не требуются.
...
Начальная Сила берётся из CardDefinition.power при каждой новой incarnation на battlefield и создаётся до снимка входа. Power = 0 намеренно допустима: карта фактически входит с нулём, создаёт входные события, затем уходит на первом checkpoint при onZero: graveyard. Входные триггеры не успевают добавить ей Силу до этой проверки; обычному автору редактор показывает предупреждение, а не ошибку схемы. Это инициализация, не исполнение place_effect_tokens и не поток effect_token_added за каждую стартовую единицу. Возвращение с кладбища даёт начальное значение, а не старые жетоны. Для шаблона types:champion onZero: graveyard: при нуле карта уходит на кладбище в R-19; это permanent_left, не смерть существа, если у типа нет категории creature.
...
#### Общий лимит активаций и пример носителя Силы
...
Общий карточный EntitySnapshot, включая PermanentSnapshot, теперь содержит typeId, categories, структурные capabilities и сериализуемые данные присутствующих компонентов. CreatureSnapshot/ResourceCardSnapshot — проверенные проекции для типизированных выражений. Смерть существа определяется переходом battlefield → graveyard плюс категорией creature по профилю; никакой другой переход не становится смертью от того, что у карты есть health.
...
Наличие свободного unit-слота не требуется для объявления эффекта возвращения: цель может быть законна на полном поле, а место может освободиться ответом или жертвой при оплате. При разрешении места может не быть — тогда эффект не перемещает карту. Это намеренная разница с прямым CastCard постоянного объекта по R-13; create_token также проверяет вместимость при исполнении.
...
Для целевого игрока используется selected из объявленных targets; селектор player означает любого игрока, enemy_player — только соперника. Для X, вычисленного по событию/жетонам, amount использует read_event/read_effect_token_count и арифметику. Неограниченный пользовательский выбор X в этой версии не вводится: конкретный пример Силы задаёт 3, другие карты задают константу или определённую формулу.
...
| core:spark | Искра | 2 | Заклинание, fast | Нанести выбранному вражескому игроку или существу 3 урона. |
| core:mend | Исцеление | 1 | Заклинание, fast | Вылечить выбранному своему игроку или существу 3 здоровья. |
...
| examples:harvest_keeper | Существо, slow, цена 4, 2/4 | Harvest_after_attack из 7.13 с выбором своего игрока или существа |
...
Дополнительные fixtures: dragon_arrival + оба TokenDefinition, Ученица с offer_self, Яйцо с hatch, abilities:fire_breathing, comfort_on_draw, attack_echo и arrival_blessing. Они проверяют совместную работу с базовыми аурами/подписками и не меняют автоматически состав исходной колоды из 12 карт. JSON-фрагменты 6.3/7.18/7.19 — тела способностей и шаблоны; раннер оборачивает их в полные определения тестовых карт с явными характеристиками. Сценарии T-103–T-123 обязательны для E8. Arrival_blessing использует apply_effect_token; добавлены effect_tokens:strength/weakness/regeneration/grounded и T-127–T-139 для E8. Разовая выдача жетонов и modify_stat — разные доступные операции; эта проверочная карта использует жетоны.
...
| duality:light_mend | 2 | 1 Свет | Быстрое лечение своего игрока или существа на 3 |
| duality:dark_spark | 2 | 1 Тьма + 1 любой | Быстрый урон 3 вражескому игроку или существу |
...
DeckDefinition использует id, name и cards: [{cardId, count}]. Count — положительное целое. Разворачивание entries идёт в порядке массива, затем копии данной записи подряд; дубликаты cardId между entries отклоняются. Expanded-список проверяется по GameConfig.deck и передаётся перемешиванию по 16.8. Авторский формат колоды — только записи {cardId, count}; ExpandedDeck — внутренний результат разворачивания, а не второй входной формат.
...
### 8.3. Проверочный пакет составных типов
...
| Первый игровой, B1 | Играть против человека: оценивать атаки/блоки, лечение, жертвы, ресурс и ответы | Проходит тактические сценарии и проверочные завершённые партии, отвечает на все виды решений; случайные прогоны ограничены и незавершённые учитываются отдельно |
...
### 9.6. Осознанные свойства профиля и проверки темпа

Эти пункты выявлены ревью v0.10. Они не изменяются незаметно ради сближения с MTG; при плейтесте можно принять отдельное изменение правил.

- **Носитель Силы и старые цели.** Types:champion не входит в enemy_character/enemy_creature. Адресный урон стартовых core/duality не выбирает его, но он может быть целью атаки и цели enemy_damageable у новой карты; контрзаклинание может отменить его карту в стеке. Это область выбора, не общая неуязвимость к эффектам. Сценарий ниже доказывает поддержку нового селектора без изменения стартовой колоды.
- **Нулевая атака.** R-15 намеренно требует effective attack > 0. Существо 1/x после −1 не может объявить атаку, но может блокировать, если остальные условия выполнены. Если атака законно объявлена, её последующее снижение до 0 не отменяет объявление/блок: наносится 0 урона.
- **Темп.** Полный ход с атакой имеет десять обычных окон приоритета; при пустом бое и в CLEANUP их число отличается. W1 обязан ясно показывать пас и причину отсутствия других действий; возможны горячая клавиша и подсказка, но скрытый автопас не вводится. Длительность 15–25 минут — гипотеза для измерения, не критерий, доказываемый числом тестов.
- **Стартовые ресурсы.** Для 10 источников в 24 картах вероятность начальной руки из 4 без источника равна C(14,4)/C(24,4) = 1001/10626 ≈ 9,42 %. Пересдача по R-03 по-прежнему отсутствует. Нужны fixtures без источника, только с источниками и с неподходящим цветом; бот делает законные действия и не зависает. Он не обязан выиграть или гарантированно завершить каждую случайную партию: остановки по лимиту учитываются как незавершённые, по 16.8.
- **Количество носителей Силы.** Базовый types:champion не занимает unit-слот. Отдельного балансного лимита пока нет, действует общий технический предел 256 permanent на игрока; две копии одного cardId в колоде не ограничивают общее число разных чемпионов. Это рабочее решение, которое оценивается плейтестом; оно не добавляет им право атаковать/блокировать.

Пример отдельной проверочной карты, не включённой в стартовую колоду:

```json
{
  "id": "examples:force_bolt",
  "name": "Удар по уязвимой цели",
  "typeId": "types:instant",
  "cost": { "generic": 1 },
  "targets": [
    { "id": "victim", "selector": "enemy_damageable", "count": 1 }
  ],
  "effects": [
...
Fixture создаёт вражеского игрока, обычное существо, types:champion и артефакт без damageable: первые три допустимы, последний — нет. Эта карта использует обычный damage и требование effect.damage; специального эффекта «урон чемпиону» не требуется.

### 9.7. Визуальная часть по этапам

Визуальная часть проектируется вместе с контрактами ядра: нужно заранее определить, какие состояния, причины и выборы видит человек. Здесь задаются состав экранов и проверяемое поведение. Художественный стиль, иллюстрации и декоративная анимация выбираются после первого плейтеста; они не являются условием E8.

**Рабочее допущение:** первая браузерная версия предназначена для компьютера с мышью и клавиатурой. Контрольные размеры W1/W2: 1366 × 768 и 1920 × 1080 при масштабе 100 %. На меньшей ширине панели переходят во вкладки; полноценная телефонная компоновка — следующее развитие, не скрытый критерий P1. Эти параметры можно уточнить до W1 без изменения правил ядра.

| Этап | Что видит человек | Зачем это нужно | Обязательность |
|---|---|---|---|
| E1–E2 | Текстовая консоль: состояния сторон, фаза, приоритет, стек, список команд/выборов | Проверить начало партии, оплату, ответы и переходы | Обязательная часть CLI по мере реализации механизмов |
| E3–E7 | Та же консоль с инспектором карты, связей, эффектов и причин отказа; сценарии со сравнением ожидания и результата | Разобрать бой, ауры, жетоны, Силу, триггеры и каскады | Обязательно для реализованных возможностей; полная проверка на E8 |
| V0, после появления состояния поля на E3 | Необязательный схематичный просмотрщик fixture/replay в браузере: прямоугольники карт, выбранный кадр, стек и журнал | Быстрее понять расположение объектов и связи без игрового интерфейса | Вспомогательный инструмент; не блокирует E8, B1 или P1 |
| E8a/E8b | Отчёт раннера: сценарий, первый несовпавший шаг, ожидаемое/фактическое, причинная цепочка; воспроизводимая запись | Подтвердить полное ядро и локализовать сбой | CLI-отчёт и replay обязательны; V0 только при наличии |
| B1 | Отчёт решений и партий проверочного/игрового бота, возможность воспроизвести выбранный эпизод | Проверить законность и объяснить плохое решение | Обязательно через существующую диагностику; отдельный красивый экран не нужен |
| W1 | Простой браузерный стол для человека и бота, а также передачи управления двум людям | Сыграть полный матч с окнами ответов и всеми типами выбора | Обязательно для P1 |
| W2 | Мастерская: каталог, формы/блоки, живая карточка-предпросмотр, проверка, испытание, колоды | Создавать контент без ручного написания JSON | Обязательно для P1 |
| W3/P1 | Связный путь «начать → собрать → сыграть → создать → испытать → экспортировать» | Проверить публичную сборку и доступность действий | Обязательно; сложная графика не требуется |

Критерии V0 применяются только при его реализации: просмотр кадра не изменяет матч, промежуточный кадр нельзя продолжить как сохранение, режим полной диагностики явно помечен.

V0 — представление данных существующих механизмов, а не альтернативный движок или ранний W1. Он не требует полноценного редактора, мышиного управления матчем или нового формата сохранений. Разработка V0 оправдана, если уже помогает разбирать сложные сценарии; при нехватке времени используется CLI.

#### Консоль: понятное состояние вместо полного дампа

После принятой команды показывается короткая сводка: кто действует, ход/шаг, кому принадлежит приоритет, что изменилось, что ожидает ответа и какие действия доступны. Полный JSON доступен отдельно для диагностики, а не заменяет основной вывод.

Пример компоновки вывода (условный кадр, не запись реально выполненного теста):

```text
Ход 4 · MAIN_1 · ход игрока 1 · приоритет у игрока 2

Игрок 1: здоровье 18/20 · Свет 1 · Тьма 0 · unit 2/7
Игрок 2: здоровье 16/20 · Свет 0 · Тьма 2 · unit 1/7

СТЕК — следующий разрешится первым
1. Тёмная искра [игрок 1] → Страж Света [игрок 2] · урон 3

ПОЛЕ ИГРОКА 2
[guard-2] Страж Света · атака 2 · здоровье 3/3 · готов
  Свойства: Бдительность
  Жетоны: нет · Чары: нет

ОЖИДАЕТСЯ РЕШЕНИЕ ИГРОКА 2
Разыграть допустимую быструю карту / активировать способность /
передать приоритет / сдаться
```

Инспектор по id раскрывает базовые/текущие характеристики, damage, броню/Силу при наличии, жетоны, сроки, источник каждого вклада, выданные/подавленные способности и причину невозможности действия. Например: «Не может блокировать: у атакующего Полёт; у блокирующего нет Полёта или Захвата». Эта причина приходит из проверок ядра; клиент не выводит её по своей копии боевых правил.

Сравнение теста показывает ожидаемое и фактическое значение, шаг команды, объект и путь assertion. Ошибка типа контента показывает id определения и путь поля. Technical_error, IllegalCommand, проигранная партия и незавершённый прогон имеют разные подписи.

#### Просмотрщик V0 и работа с историей

V0 отображает уже записанный fixture/replay/trace: список событий слева, выбранное состояние поля в центре, подробности объекта и стека справа. Переключение кадров не отправляет игровые команды. Если на E3 ещё не готов replay, достаточно поддержанного снимка fixture и сравнения до/после команды.

Полное состояние debug и обычный PlayerView — разные источники. В диагностике режим с открытыми руками явно помечается «Тестовый просмотр: все данные». Обычный матч человека и наблюдение бота используют только разрешённые данные; инструменты диагностики не подмешивают скрытые поля в BotDecisionContext.

Шаг просмотра по умолчанию — стабильная граница принятой команды/решения. Внутренние кадры damage/checkpoint при наличии trace доступны только для чтения и помечены как промежуточные: на них нельзя продолжать матч. Возврат курсора к прошлому кадру не означает отмену оплаченного действия. Повтор проверки запускается из fixture с тем же seed/снимком/командами; редактирование произвольного GameState в работающей партии не вводится.

#### W1: компоновка игрового стола

```text
┌─────────────────────────────────────────┬────────────────────────┐
│ ПРОТИВНИК: здоровье · ресурсы · рука N   │ Ход / шаг / приоритет │
│ Закрытая рука · колода · кладбище       │ Что сейчас требуется  │
├─────────────────────────────────────────┤                        │
│ Источники и постоянные объекты врага    │ СТЕК                  │
│ Боевой ряд противника                  │ Следующая способность │
│                                         │ Цели и источник       │
│ Атакующие → цели / назначенные блоки    ├────────────────────────┤
│                                         │ ИНСПЕКТОР             │
│ Ваш боевой ряд                         │ Выбранная карта       │
│ Ваши источники и постоянные объекты     │ Свойства / жетоны     │
├─────────────────────────────────────────┤ Источники бонусов     │
│ ВАША РУКА                              ├────────────────────────┤
│ Здоровье · ресурсы · зоны · unit N/7    │ ЖУРНАЛ                │
│ [Действие] [Подтвердить] [Передать ...]  │ Последние события     │
└─────────────────────────────────────────┴────────────────────────┘
```

Это схема размещения, а не окончательная художественная компоновка. На узком окне инспектор/журнал становятся вкладками, приоритет и текущий запрос остаются видимыми. Текст длинной карты читается в инспекторе без необходимости уменьшать весь стол.

Расположение объекта выбирается по слотам и CompiledCardType: боевой ряд — unit-slot; источники — capability source_deploy; другие permanent — отдельная область. Один объект отображается одной основной карточкой: источник-существо расположен в боевом ряду и имеет доступное производство, а не дублируется в двух рядах. Носители Силы показывают возможность быть целью атаки. Чары представлены собственной доступной карточкой/строкой связи у носителя; по выбору видны обе стороны AttachmentBinding.

Публичные кладбище и изгнание открываются списком. Колода и чужая рука показывают разрешённый счётчик/рубашки, без раскрытия названий. Большое поле не должно уменьшать текст до нечитаемого: для рядов и списков предусмотрены прокрутка/раскрытие групп, видимые счётчики и переход к выбранному объекту. Основная проверка проводится при unit-лимите 7, стресс-проверка — также при 32 и множестве других permanent.

#### Что показывает отдельная карта

- Название, категории и виды, цена по типам ресурсов, текст и список способностей.
- Только существующие показатели: атака, текущее/максимальное здоровье, броня, Сила. У артефакта без health/attack не появляется выдуманное «0/0».
- Готовность/повёрнутость и запрет атаки в ход входа — отдельные подписи/значки. Можно слегка наклонять рамку, но текст остаётся читаемым.
- Жетоны, например «Золото ×2», с раскрытием отдельных экземпляров/сроков; числовое объединение виджета не объединяет их состояние в ядре.
- Эффекты ауры, прикреплённые чары и собственные жетоны различаются. Инспектор объясняет «+1 атаки от Знамени» отдельно от «+1/+1 от жетона».
- Активируемые способности имеют свои кнопки со стоимостью и общим лимитом группы. Причина недоступности видна по фокусу/выбору, а не только по наведению мыши.

Ресурсы игрока показываются по resourceId с названием/символом/числом. Свет и Тьма — первый набор, а не фиксированное количество цветных ячеек интерфейса. Новые категории/ресурсы/свойства получают текстовое представление по метаданным и стандартную рамку, если специальных иллюстраций нет.

#### Объявление действия, оплата и ответ

Розыгрыш собирается как черновик команды: карта/способность → цели → варианты оплаты/жертвы/жетоны → сводка → одно подтверждение. UI показывает только доступные для выбора варианты по getSelectionOptions и проверяет актуальность revision. Пока команда не отправлена, выбор можно изменить/отменить; карта не покидает руку и ресурсы не списываются.

Сводка явно различает выбранные цели и стоимость: например «Повернуть Сокровище, снять 1 Золото; при разрешении взять карту». После принятия команды оплата завершена, действие может ожидать ответа в стеке. Отмена экрана не возвращает платёж. Повторное нажатие подтверждения во время обработки не посылает вторую команду.

PendingDecision показывает, чей выбор нужен, какой выбор необходим и в каком количестве и почему остальные недоступны. Закрытие панели не отменяет обязательный выбор и не открывает посторонние действия. До принятия SubmitChoice можно редактировать выбор; сдаться может любой участник по R-08. При ошибке команды сохраняется принятое ядром состояние и объясняется причина отказа.

«Передать приоритет» не подписывается как «закончить ход». Игрок видит следующий эффект стека и кому передаст решение. Бот имеет индикатор размышления, но человеческое окно ответа не исчезает из-за анимации или смены панели. Атаки и блоки задаются выбором карт/целей и подтверждаются всем набором; drag-and-drop необязателен.

Для двух людей действует экран передачи управления: прежняя рука/инспектор скрываются до показа новой стороны. Защита распространяется на списки выбора и журнал с приватными полями, а не только на ряд карт. При переключении обновляется PlayerView и очищаются приватные панели/черновики предыдущего игрока; возврат к вкладке или прежнему выбору не раскрывает его руку. В режиме против бота экран передачи не появляется.

#### W2: мастерская и испытание

```text
Каталог                 Форма карты                    Предпросмотр
Карты                   Тип / название / цена          Карточка
Типы                    Доступные характеристики       Текст способности
Способности             Свойства и способности         Ошибки рядом с полем
Жетоны / фишки          Цели → условия → эффекты        [Проверить]
Ресурсы / колоды        События / стоимость / сроки     [Испытать]
Сценарии                Дополнительные параметры       [Сохранить/экспорт]
```

Автор обычной карты выбирает готовый тип и получает только подходящие поля. Конструктор типов/общих способностей находится в отдельных разделах, чтобы создание существа не требовало каждый раз собирать компоненты. JSON — дополнительное представление для опытного автора; формы и JSON сохраняют одну модель и проходят один валидатор.

Предпросмотр показывает черновик, но не обещает его законность до проверки. Печатный текст и фактическая программа различаются в редакторе; автоматически собираемая подсказка берётся из проверенного AST. При изменении общей способности/типа показан список зависимых карт, которые будут перепроверены.

«Испытать» использует два режима 9.5: сценарий или партия с ботом. Сценарная форма задаёт исходные зоны/ресурсы/шаг/объекты и ожидаемые утверждения; debug-просмотр явно отделён от обычной игры. Исправление черновика не меняет запущенный тест — нужен новый запуск. После теста доступны результат, первый несовпавший assertion, журнал и возврат к конкретной карте.

#### Визуальный стиль и проверка удобства

Для W1 достаточно нейтрального фона, текстовых карточек, устойчивой сетки и простых значков. Сначала проверяются читаемость и полный игровой цикл. Для P1 нужны согласованные кнопки/подписи/цвета, состояния загрузки/ошибки и понятное начало матча; дорогие иллюстрации, звук и эффекты не обязательны.

Значение не передаётся только цветом: есть подпись/значок/форма выделения. Все обязательные действия доступны кликом и клавиатурой, выбранные цели и фокус заметны. Подсказки раскрываются также по фокусу/клику. Анимации, если появятся, не определяют момент оплаты/урона, могут быть сокращены/отключены и не меняют состояние либо RNG.

На E8 проверяется диагностика состояния и воспроизведения. На W1/W2 дополнительно проверяются: все типы выбора, длинные названия/тексты, заполненные ряды, несколько чар/жетонов, новые ресурсы, клавиатурный маршрут, отсутствие скрытых данных в PlayerView и сохранение контекста при ошибке. Снимки экранов полезны для компоновки, но не заменяют проверку игровых команд/состояния. Корректность расчётов подтверждается сценариями и assertions.
...
- engine/abilities: AbilityDefinition bindings, AbilityInstance, параметры, маски, происхождение, unique/per_source и компиляция inline-определений.
...
- content: определения, схемы, capability-реестр, словари видов/признаков, KeywordDefinition как индекс, AbilityDefinition и каталоги привязанных правил, RulesetManifest/InteractionRule, проверка версий.
...
Если реализуется V0, его адаптер просмотра использует существующие снимки/события и не добавляет правил; обязательные интерфейсы E8 остаются в CLI.
...
Состояние Git (ветка, HEAD, remotes, рабочее дерево) проверяется при начале реализации и не закрепляется хешем в нормативной спецификации. Перед кодом проверить актуальное состояние, настройки и выбрать рабочую ветку. Хранилище Obsidian автоматически не перемещать.
...
|---|---|---|
| E0 | Спецификация 0.14 | Контракты ядра, библиотека способностей, события/фишки/лимит поля/жетоны эффектов/оба режима экономики/составные типы/чары/Сила/перемещения, E8/P1, бот, мастерская, колоды и PR описаны; визуальные этапы и критерии 9.7 согласованы с CLI/E8/P1; конвертеры/сеть не расширяют первый выпуск |
| E1 | Типы, схемы, состояние и каркас CLI/раннера | ComponentRegistry/CardTypeDefinition/CompiledCardType/categories, AbilityDefinition/TokenDefinition/EffectTokenDefinition/ref/inline, EventDefinition, GameConfig/EconomyConfig, ResourceDefinition/ResourceCost/PaymentPlan, hostRequirements/activeZones/activeOnPlayer/params, связывание текущего формата, каталоги/dependencies, команды, RNG, fixtures и assertions |
...
| E3 | Бой, InteractionRule и checkpoint | Чистые предикаты признаков, AND ограничений, атомарные декларации, фиксация блоков, выбор defender по attack_target, броня/health/power, одновременный урон и общие волны ухода; проверка через CLI, V0 необязателен |
...
| E6 | Ауры и экземпляры способностей | Grant/suppress_ability, индекс keywordId, unique/per_source, EffectTokenDefinition/Instance, apply/remove/count, события и сроки жетонов, разрешённые выданные continuous, AttachmentBinding/attached_to, маски, происхождение и каскады |
...
| V0 (необязательный) | Просмотрщик тестовых состояний после E3 | Кадры/стек/связи/журнал по 9.7; просмотр без изменения партии, debug явно помечен; отсутствие V0 не блокирует E8/B1/P1 |
| E8a | Интеграция базового игрового цикла | Canonical-типы E1, CLI, приоритет/стек/бой/DSL/ауры и baseline fixtures; воспроизводимость сценариев уже реализованных подсистем |
| E8b | Интеграция полного состава ядра | Оба режима экономики, чары/Сила/броня/жетоны/воскрешение, взаимодействия и все сценарии приёмки E8; replay и по 100 прогонов бота на режим; читаемый отчёт и локализация первого несовпадения по 9.7 |
| E8 | Полная приёмка ядра | E8a и E8b завершены, все критерии раздела 12 выполнены; готовность одного среза не означает готовность всего ядра |
...
Порядок основной реализации E1 → E2 → E3 → E4 → E5 → E6 → E7 → E8a → E8b; E8 — их итоговый критерий, а не ещё один повтор всех прогонов. Сценарии E8a можно собирать по мере готовности E1–E6, но E8b не откладывает типизацию: её каркас обязателен на E1. Объём включает несколько самостоятельных подсистем; календарный срок до оценки реализации не обещается. На каждом E-этапе нужен демонстрируемый консольный срез; новые необязательные механики не расширяют этот выпуск автоматически. Минимальные чистые предикаты признаков/and/or для InteractionRule реализуются к E3; E4 расширяет их до полного словаря выражений и фильтров способностей. Проверки и CLI-поддержка каждой возможности делаются вместе с ней. B1/W1–W3 не являются условиями E8, но обязательны для P1. После полной E8: B1 → W1 → W2 → W3 → P1; формы можно разрабатывать по уже стабильным метаданным одновременно с интеграцией клиента. V0 вне основной цепочки: его можно делать после E3, только если он помогает проверкам; он не заменяет CLI и не задерживает E8. I1 и графовый редактор со стрелками не блокируют P1. Новую механику нельзя переносить за E8 только потому, что её неудобно показать в текущем UI.
...
| T-71 | HostRequirements и activeZones: Полёт у one_shot, stat-поведение у игрока | Неподходящие носители отклоняются; выдача Полёта совместимому permanent законна; activeOnPlayer не создаёт карточную зону. |
...
| T-147 | Источники на заполненном поле существ | Источник не занимает maxCreaturesPerPlayer и не является целью creature/character-эффектов; общий READY разворачивает все свои permanent с exhaustion, включая артефакт/чары; объекты без exhaustion не получают tapped. |
| T-148 | Существо и resource с одной ресурсной активацией | Новое существо не платит tap до готовности; новый resource может. Повторный tap обоих отклоняется; ресурсная способность соблюдает hostRequirements. |
...
| T-161 | ResourceCost и spend_resource с generic/resources | Generic оплачивается выбранным ресурсом, typed light не превращается в energy; числовой cost и amount у spend_resource отклоняются как неверная форма. |
...
| T-174 | Единый формат core/duality и проверка typeId | Оба набора используют актуальные поля, typeId разрешается из каталога; неизвестный тип/версия/поле отклоняются без преобразования. |
...
| T-197 | Объект с health и power / нулём health у не-существа | Damageable.sink задан однозначно; лечебные эффекты не меняют Силу; уход по health не создаёт смерть существа без нужной категории. |
...
| T-204 | READY повёрнутых существа, источника, артефакта и чар | Все свои exhaustion-носители разворачиваются; чужие и объекты без компонента не изменяются. |
| T-205 | CLEANUP у health-носителя вне категории creature | MarkedDamage снимается, временные эффекты истекают; броня и Сила не восстанавливаются. |
| T-206 | DeathRecord с каскадом и zero_power/invalid_attachment | Reason/reasons из одного списка, checkpointId/stateCheckWave отдельно; DeathRecord только для категории creature. |
| T-207 | Лимиты behavior/ref/definition/экземпляров | Граничные 256 узлов/16 глубины, 32 behaviors/4096 узлов определены однозначно; AST не умножается экземплярами, runtime-работа учитывается каждый раз. |
| T-208 | Requires: подписка, испускание, нужный и лишний пункт | Явная подписка требует event-capability; испускание покрывает операция. Duality не требует event.resources/отдельную capability InteractionRule; пропуск/неподдерживаемый id — ошибка, известный лишний — предупреждение. |
| T-209 | Selector.filter и collection filter с вложением | Candidate и именованный item имеют разные области; неверные имена/результат не Boolean отклоняются. |
| T-210 | DurationSpec и обязательный object у is_damaged | Объект duration и object предиката работают; строковый срок, неявный candidate и неизвестные поля/варианты отклоняются. |
| T-211 | On_enter на многокатегорийном типе, on_death на не-существе | Один канонический вход, нужные проекции; on_death без доказанной категории creature отклоняется при связывании. |
| T-212 | Актуальный формат всего проверочного контента | Карты duality, учебные карты, fire_breathing и все жетоны используют typeId/hostRequirements и текущие цены; импорт и экспорт одного формата без дополнительных режимов. |
| T-213 | Equal ссылочных подтипов и effective attack < 0 до ограничения | CombatantRef и CreatureRef одного объекта равны; разные generation не равны; effective/read_event/snapshot.attack возвращают 0. |
| T-214 | Отложенная реакция вне активного CombatScope | Не создаёт подписку/событие/technical_error и не срабатывает в следующем бою; внутри scope срабатывает один раз. |
| T-215 | Вход types:champion с power = 0 | Вход фиксируется, на первом checkpoint следует уход; on_enter не успевает спасти источник добавлением Силы. |
| T-216 | Возвращение при полном поле с последующим освобождением места | Объявление законно; ответ/оплата может освободить слот, повторная проверка при разрешении определяет фактический вход. |
| T-217 | Concede в чужой приоритет и чужой PendingDecision | Участник может сдаться на стабильной границе; отменяется ожидание решения, дальнейшее разрешение прекращается. |
| T-218 | Носитель Силы и разные селекторы/лимиты | Старый enemy_character его не выбирает; enemy_damageable выбирает, урон снимает Силу; unit-слот без компонента не занят. |
| T-219 | Атака 1 после −1 до/после объявления | До объявления атака запрещена; после объявления атакующий остаётся в бою с 0 урона, блокирование само по себе не запрещено. |
| T-220 | W1: окна без быстрых действий | Пас заметен и доступен; UI не пропускает окна скрытно, ответы человека сохраняются. |
| T-221 | B1: рука без источников/только источники/не тот цвет | Законные решения, отсутствие зависаний и автоматической сдачи; истечение лимита записывается как незавершённый прогон. |
| T-222 | W2: термины целей и диагностика канонического формата | Показываются «игрок или существо», «носитель Силы» и список реально допустимых целей; ошибка содержит JSON-путь. |
| T-223 | E8: диагностика CLI и несовпавшего сценария | Краткая сводка показывает приоритет/стек/ожидаемый выбор; инспектор раскрывает причины запрета и происхождение эффектов. Намеренно неверный assertion локализован до шага/объекта/поля с ожидаемым и фактическим; IllegalCommand, technical_error, проигрыш и незавершённый прогон различаются. |
| T-224 | W1/W2: читаемость при разных размерах и заполненном поле | При 1366 × 768 и 1920 × 1080 (100 %) доступны приоритет/выборы, длинный текст и все объекты при лимите 7 и 32. Составной источник-существо показан один раз; новые типы ресурсов имеют подписи и количество. |
| T-225 | W1: черновик действия, отмена и повторное подтверждение | До отправки состояние не меняется; цели и стоимость различимы; одно подтверждение даёт одну команду. После принятия нельзя отменить оплату закрытием панели; отказ не меняет принятое ядром состояние и показывает причину. |
| T-226 | W1: передача управления и обязательный выбор | У предыдущего игрока очищаются приватные рука/инспектор/выбор/журнал; после передачи доступны только данные нового PlayerView. Закрытие PendingDecision не отменяет его и не разрешает посторонние команды; Concede остаётся доступен обоим участникам. |
| T-227 | W1/W2: управление без перетаскивания и различения цветов | Обязательные действия выполняются кнопками и клавиатурой, фокус/цели заметны; подсказки и причины отказа доступны по фокусу/клику. Состояния различимы по подписям/значкам; если анимации есть, их отключение не меняет результат команд. |
| T-228 | W1: аура, чары, жетоны и подавление на одной карте | Видны разные источники эффектов, сроки и отдельные экземпляры жетонов; интерфейс не объединяет их в состоянии ядра. У объекта без health/attack нет фиктивного 0/0; инспектор объясняет текущие характеристики и выданные/подавленные способности. |
...
**Приёмка MVP ядра:** E1–E7, E8a и E8b завершены (вместе E8); T-01–T-23, T-25–T-83, T-103–T-123, T-127–T-139, T-142–T-164, T-169–T-198, T-204–T-219 и T-223 пройдены. Наборы core/duality и механические fixtures 8.1–8.3 работают из JSON; сложные сценарии проходят в консоли/раннере, snapshot/replay повторяет состояния и события. Все технические остановки и незавершённые прогоны разобраны, а не засчитаны победами/ничьими. Браузер (включая необязательный V0), графика и T-24 не блокируют приёмку; T-24 выполняется на W3.

**Приёмка первой публичной версии P1:** E8, B1 и W1–W3 завершены; дополнительно пройдены T-24, T-84–T-100, T-124–T-126, T-140–T-141, T-165–T-168, T-199–T-203, T-220–T-222 и T-224–T-228. Бот отвечает на все виды решений и проходит зафиксированные тактические fixtures; редактор создаёт обычные карты/способности без ручного JSON; тест/экспорт/PR-процесс воспроизводимы. Оценка силы/баланса продолжается после выпуска. T-101–T-102 относятся к I1 и не блокируют E8/P1.
...
3. Повысить редакцию документа и записать изменение в [[MagicCards-history]]. При изменении формата повысить schemaVersion, при изменении правил — rulesetVersion, при изменении контента — packVersion; при чисто редакционной правке эти три версии не меняются. Отметить влияние на будущие сохранения/replay; поддержку прошлых черновиков не добавлять.
4. При изменении поведения добавить или изменить T-* с ожидаемым результатом; при редакционной правке проверить согласованность существующих сценариев.
...
| D-45 | Уточнено в v0.12 | Единый актуальный формат DurationSpec/object/typeId/hostRequirements. Requires покрывает используемые возможности, известные лишние допустимы с предупреждением. Лимиты считаются на behavior и definition после разрешения AST-ссылок. |
| D-46 | Рабочие правила v0.11 | Concede любым участником на стабильной границе; power = 0 допускает вход с последующим уходом; delayed combat-trigger вне активного боя ничего не создаёт. Балансные особенности 9.6 сохранены, а не признаны ошибкой. |
| D-47 | Требование владельца v0.12 | До реализации не поддерживать форматы предыдущих черновиков: нормативные описания и примеры сразу актуальны. Версии/снимки сохраняются для будущей воспроизводимости; история проектирования хранится отдельно. |
| D-48 | Рабочее предложение v0.14 | Визуальные этапы 9.7: обязательная читаемая CLI-диагностика для E8, необязательный просмотрщик V0, полноценные выборы браузерного стола W1 и мастерская W2. Для первого P1 предложен компьютер с мышью/клавиатурой; точные размеры и компоновка уточняются перед W1, иллюстрации/анимации и телефонная компоновка не блокируют P1. |

Источники контекста:

- Проектное обсуждение движка карточной игры — замысел и варианты архитектуры.
- Проектное обсуждение организации документации — хранение спецификации рядом с кодом.
...
Журнал редакций находится в [[MagicCards-history]]; он фиксирует ход проектирования и не определяет поддерживаемые форматы.
...
Concede проверяет принадлежность actor к матчу и отсутствие результата, но не требует приоритета или права на текущий выбор. Внешние команды сериализуются: если запрос пришёл во время исполнения frame, он обрабатывается на следующей стабильной границе; завершившийся матч уже не меняется.

PendingDecision — законная точка фиксации. В этом состоянии доступен SubmitChoice нужного игрока либо Concede любого участника; обычные карты/активации/пасы отклоняются. Уточнение выбора не меняет значения уже оплаченных стоимостей. Невалидный ответ оставляет pendingDecision неизменным.
...
2. На одном снимке определить проигравших игроков и все permanent, подлежащие уходу: lethal_damage/zero_max_health у health, нулевая Сила с onZero: graveyard, недопустимая обязательная связь чар. Для state-based ухода хранить reasons и выбирать reason по порядку lethal_damage → zero_max_health → zero_power → invalid_attachment. Lethal_damage требует maxHealth > 0 и markedDamage ≥ maxHealth; при maxHealth ≤ 0 используется zero_max_health. В каждой волне записывать checkpointId/stateCheckWave, по общему контракту 7.13; явные destroy/sacrifice оформляются в своих операциях.
...
**Единый DurationSpec.** В каноническом JSON поле duration всегда объект: {type: "until_end_of_turn"}, {type: "permanent_on_instance"} либо {type: "until_source_leaves"}. Имя варианта в таблице/прозе — сокращённое описание, не строковый JSON. Строковая форма не входит в схему и отклоняется. Until_next_controller_turn_start в 7.14 — будущий вариант статусов, а не поддержка этой механики в текущем ядре. Допустимые варианты дополнительно ограничены операцией: create_continuous_effect поддерживает только until_end_of_turn; EffectTokenInstance — сроки, перечисленные в 7.19. Наличие общей формы duration не открывает все варианты каждой операции.
...
Разовые modify_stat и modify_keyword создают ModifierContribution с duration = until_source_leaves / until_end_of_turn / permanent_on_instance. Modify_stat явно указывает duration; permanent_on_instance означает действие до выхода **получателя** с поля. Until_source_leaves требует источник на поле и привязан к конкретной incarnation; уход и повторный вход его не восстанавливают. Использование этой длительности с обычным заклинанием вне поля отклоняется семантическим валидатором.
...
AST-счёт R-22: узел behavior считается корнем; каждый effect, cost, predicate, expression (включая нормализованный literal/reference), modifier и selector-query — отдельный узел. Контейнеры массивов, имена полей, id, текст карты и параметры схемы узлами не являются. Depth — максимум по вложенным семантическим узлам, корень имеет глубину 1. Вложенные afterResolve/delayed-программы учитываются внутри создающего behavior; Ссылка на исполняемый behavior/rule разворачивается на каждом месте использования; цикл такого AST запрещён. Ref на создаваемый TokenDefinition остаётся типизированной ссылкой контента: его возможная обратная ссылка через будущий триггер обходится visited-set и не встраивает всё будущее исполнение в текущий AST. Повторные AbilityInstance используют общий проверенный AST, но каждое фактическое исполнение тратит runtime-бюджет. Предел модификаторов — дополнительное ограничение, не обещание, что 64 произвольно сложных модификатора уместятся в лимит узлов.

Помимо R-22: 128 участников foreach, 64 modifier-записи на один continuous-behavior, не более 4096 активных вкладов, 64 жетонов эффектов на одном носителе и 1024 в матче, 256 независимых ContinuousEffectInstance, 128 InteractionRule на одно взаимодействие и 256 ожидающих триггеров. Каждая проверенная вершина выражения, шаг обхода, попытка добора, создание/удаление жетона и его компонента, применение эффекта, проверка InteractionRule и волна checkpoint учитываются в бюджете внешней команды. Превышение — technical_error с диагностикой, а не зависание.
...
Первый клиент ядра — Node.js-консоль. Пример читаемого вывода, требования к инспектору и локализации сбоев приведены в 9.7. Установочные и запускные команды задаются в реестре при реализации; приведённые ниже слова — проект интерфейса консоли, не готовые shell-команды.
...
Все fixtures задают presetId/economy и типизированный пул; автоматический режим использует automatic_energy_v1 и pool[core:energy]. Смена default приложения не меняет явно заданные параметры сценария.
