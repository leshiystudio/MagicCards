...
**Версия:** 0.9 · **Дата:** 15.09.2026 · **Статус:** исходная спецификация; код ещё не реализован.
...
**Рабочие допущения v0.9:** TypeScript и браузерный стек; ограниченная модель блоков; числа баланса, размеры колод/руки, правила производства/хранения и стартовые карты ниже. Это предложения для старта, а не ранее утверждённые решения. Их изменения оформляются следующей редакцией.
...
| R-02 | Колода | Ровно 24 карты; максимум две копии одного cardId, кроме resource-источников из явного unlimitedCopyCardIds пресета. В deck_sources допустимы creature/spell/resource, в automatic_growth — creature/spell с совместимой ценой. Колоды по умолчанию — 8.2 (Свет/Тьма); старые core 12 × 2 — для automatic_growth. Исходный порядок фиксируется, затем перемешивается генератором матча. |
...
| R-06 | Лимиты | До 10 карт в руке; существ на поле каждого игрока — не больше GameConfig.limits.maxCreaturesPerPlayer, по умолчанию 7. Добор в полную руку отправляет карту на кладбище без триггера смерти и card_drawn. Создание фишек ограничивается свободными местами при разрешении, 7.18. |
...
Эффективный GameConfig после нормализации фиксируется в снимке матча и replay; настройки будущей партии не меняют уже начатую. Изменение лимита посреди матча в первом ядре не поддерживается. Все проверки полного поля, включая розыгрыш существ R-13, create_token, сценарный раннер, UI и допустимые действия бота, читают одну настройку. Существующие тесты без явной настройки используют 7. Во время создания фишек занятость считается по фактически находящимся на поле существам обеих форм происхождения.
...
**R-12. Скорость и объявление.** Медленные карты и slow-активации играются только активным игроком в MAIN_1/MAIN_2, с приоритетом и пустым стеком. Все существа медленные. Type: resource выкладывается отдельным PlayResourceCard по 7.20 без обычного cost/speed/стека. Быстрые заклинания играются в любом окне со своим приоритетом, включая чужой ход.
...
| on_enter | Собственный вход носителя; алиас creature_entered с creature = source | Тот же payload |
...
| attackers_declared | Принят весь набор атакующих в этом бою | player, attackers: Collection<CreatureRef>, combatId |
| creature_attacked | По одному факту на каждого объявленного атакующего | player, attacker: CreatureRef, attackerSnapshot: CreatureSnapshot, defender: PlayerRef, combatId |
...
Для существа on_enter/on_death — алиасы одного факта, а не второе событие рядом с creature_entered/creature_died. Для resource-носителя on_enter выбирает resource_entered, а on_death не поддерживается по 7.20. Если автор намеренно прикрепил две разные способности, обе могут сработать. Само нормализованное определение срабатывает один раз.
...
| apply_effect_token | definitionId; target: существо/множество; amount: Integer ≥ 0 на получателя; duration | Прикрепить независимые жетоны к зафиксированным получателям по 7.19 |
| remove_effect_token | definitionId; target: существо/множество; amount: Integer ≥ 0 на получателя | Снять самые ранние экземпляры указанного жетона, без снятия других эффектов, 7.19 |
| destroy | target: CreatureRef/ResourceCardRef или разрешённое множество | Существо: смерть; resource: resource_left без DeathRecord; перемещение с поля на кладбище |
...
| modify_stat | target: существо или Collection<CreatureRef>; stat: attack/max_health; amount: Integer; duration | Знаковый баф/дебаф через вклад модификатора, правила 16.7 и 7.19 |
...
| grant_ability | target: creature/player/resource или множество; ability: ref/params либо inline; duration | Создать AbilityInstance, проверить appliesTo и stacking; ограничения выдаваемых continuous по 7.17 |
| suppress_ability | target: creature/player/resource или множество; definitionId; duration | Маскировать все выдачи данного определения на выбранных incarnation; не отменять уже созданные StackItem, 7.17 |
...
5. Для обычной activated в одной транзакции создать StackItem способности с sourceSnapshot, targets и CostReceipt; применить повороты, списание ресурса/жизней, затем одновременно жертвы.
...
Редакция документа 0.9 добавляет два режима экономики, ResourceDefinition/ResourceCost, resource-карты и основной набор duality: rulesetVersion = 0.9, schemaVersion = 2. Механика жетонов v0.8 сохраняется. Расширения v0.7 (фишки, каталог событий, настройка R-06) сохраняются. R-02 дополнен исключениями копий базовых источников и проверкой совместимости режима; базовый бой сохранён; уточнено, что card_drawn означает успешный добор в руку. Это проектные версии до первой реализации. Старый формат schemaVersion 1 нормализуется явным адаптером только для ранее описанных базовых карт: числа → Literal во внутреннем AST, отсутствие kind при trigger → triggered. Новые узлы/стоимости/ауры/ресурсы требуют schemaVersion 2. Числовая цена и старый spend_resource.amount сохраняют однозначную нормализацию в generic по 7.20, но не меняют старые версии replay. Исходные данные и применённая нормализация фиксируются при загрузке; старый replay не пересчитывается автоматически по новым правилам.
...
Фишки включены в первый вариант ядра E8. TokenDefinition — шаблон создаваемого существа с общими полями характеристик/видов/способностей. CardInstance и экземпляр фишки используют общую модель существа на поле; отдельный движок боя, аур или способностей для фишек не создаётся. Отличия — isToken, способ появления и жизненный цикл вне поля.

#### Шаблон, экземпляр и операция create_token

TokenDefinition имеет id, name, type: creature, attack, health, subtypes/tags/colors, keywords и abilities. Пустые коллекции можно опустить; они нормализуются так же, как у карты. Поля стоимости, скорости розыгрыша и корневые spell-effects отсутствуют. Шаблон не является допустимой позицией колоды. AbilityDefinition для фишки проверяется как для creature, а не как для отдельного типа token.
...
EffectTokenDefinition хранится в библиотеке набора. Поля: id, name, appliesTo: ["creature"], components. В первой реализации носителями служат обычные существа и фишки существ; игрок/заклинание не допускаются. Пустой components разрешён для видимого маркера, который сами правила могут посчитать. Определение само по себе ничего не выдаёт: нужна операция apply_effect_token.
...
В EventDefinition добавляются effect_token_added и effect_token_removed. Они создаются по одному на фактически добавленный/снятый экземпляр, а не на попытку/нулевое действие. Payload: host: CreatureRef уходящей или текущей incarnation, hostSnapshot: CreatureSnapshot, effectToken: EffectTokenSnapshot, reason: applied/dispelled/expired/host_left, batchId, causeId. Для added reason = applied; для removed — одна из трёх причин удаления.
...
Для уже написанных примеров numeric cost: N нормализуется в generic: N, а spend_resource с прежним amount: N — в generic: N. Одновременные amount и resources/generic отклоняются. Это не конвертация тематических цен: существующая цена с explicit light/dark не превращается в energy при переключении режима. В исходном automatic_energy_v1 generic расходуется единственным energy и поведение старых карт сохраняется. Эта нормализация записывается в snapshot/rulesetVersion 0.9; старые replay продолжают прежние правила.
...
|---|---|---|
| E0 | Спецификация 0.9 | Контракты ядра, библиотека способностей, события/фишки/лимит поля/жетоны эффектов/оба режима экономики, E8/P1, бот, мастерская, колоды и PR описаны; конвертеры/сеть не расширяют первый выпуск |
| E1 | Типы, схемы, состояние и каркас CLI/раннера | AbilityDefinition/TokenDefinition/EffectTokenDefinition/ref/inline, EventDefinition, GameConfig/EconomyConfig, ResourceDefinition/ResourceCost/PaymentPlan, appliesTo/activeOn/params, нормализация, каталоги/dependencies, команды, RNG, fixtures и assertions |
| E2 | Фазы, стек, приоритет и базовая экономика | Два паса, ответы, скорости, READY/границы хранения обоих режимов, PlayResourceCard, общий frame on_resolve, имена целей и повторная проверка; неверные команды атомарны |
...
| E4 | Выражения, фильтры и триггеры | Арифметика, let/filter/count/sum/foreach, фильтры видов/keyword/contact, eventSnapshot, вход/добор/атака, create_token и адресные триггеры с SubmitChoice |
| E5 | Активации, производство и стоимости | Несколько abilityId, CostPlan/Receipt/PaymentPlan, exact/generic, resource_activated/productionOptions/gain_resource, resource/life/tap/sacrifice (включая self), лимиты и снимки оплаченных объектов |
| E6 | Ауры и экземпляры способностей | Grant/suppress_ability, keyword-адаптеры, unique/per_source, EffectTokenDefinition/Instance, apply/remove/count, события и сроки жетонов, разрешённые выданные continuous, маски, происхождение и каскады |
...
| E8 | Приёмка первого ядра | CLI, fixtures/core/duality, сценарии обоих режимов, replay и по 100 прогонов проверочного бота на каждый режим с разбором остановок |
| B1 | Первый игровой бот | Все виды решений, оба режима экономики/выбор платежа, тактические fixtures, положенная информация, policy seed/бюджет и резервный выбор; сценарии 9.3–9.4 |
| W1 | Браузерный матч против бота | Стол/стек/выборы, переключатель экономики до матча, пулы/источники/лимит выкладываний, coordinator и worker, save/load; человек сохраняет все окна ответа |
| W2 | Редактор и мастерская | Формы/блоки карт, ResourceDefinition/ResourceCost/productionOptions, TokenDefinition, EffectTokenDefinition и AbilityDefinition; различение ауры и выдачи жетона, типизированные поля событий/снимки, настройка лимита матча, колоды, локальные черновики, «Испытать», импорт/экспорт своего формата и round-trip |
...
CostPlan рассчитывается полностью до изменения состояния; CostReceipt хранит ResourceCost, actualSpent по типам из PaymentPlan, суммы остальных платежей и снимки объектов до оплаты. Неповёрнутый источник не считается доступным ресурсом пула; активации производства принимаются отдельными командами до объявления оплаты (7.20). Для каждого sacrifice указано соответствие costId → выбранные жертвы. Суммирование одинаковых ресурсов обязательно, чтобы две цены по 2 не прошли при доступных 3.
...
GameEvent содержит eventId, type, turnId, phaseId, frameId, causeId, combatId при наличии и типизированный payload по EventDefinition (6.2). Для событий автоматического шага без StackItem frameId относится к системному frame этого шага; причина указывает системную операцию. EventDispatcher индексирует AbilityBinding/behaviorId по типу и области активности. PendingTrigger сохраняет снимок события и носителя, контроллера, параметры и происхождение при срабатывании, а не восстанавливает их из текущего поля при последующем помещении в стек. Идентификаторы монотонны и не расходуют RNG. Текстовый журнал — представление событий, а не данные, по которым ядро определяет правила.
...
Подписки проверяются по состоянию, предусмотренному конкретным событием. При входе на поле сначала применить перемещение и пересчитать EffectiveView, затем собрать creature_entered и собственные алиасы on_enter по состоянию после всей партии входа (6.2). Новая аура может дать вошедшему существу способность, но не создаёт повторные on_enter у уже находящихся на поле получателей. Для одновременной смерти — снимок до удаления, в том числе предоставленные аурой способности. Набор сработавших определений копируется в pendingTriggers и больше не зависит от того, сохранилась ли подписка.
...
EffectTokenInstance по 7.19 — явный независимый источник вкладов с фиксированным hostRef. Все компоненты одного экземпляра создаются/снимаются вместе, по origin effectTokenId + componentId + hostRef; выход applier не снимает их. Порядок расчёта задан в 7.7/7.17. Удаление жетона снимает только его вклады/привязки, а не меняет CardDefinition и не отматывает уже произошедшие эффекты. События added/removed создаются при изменении состава экземпляров, а не при пересчёте. Для истории смерти CreatureSnapshot/DeathRecord хранит EffectTokenSnapshot до ухода, независимо от последующего удаления носителя/фишки.
...
Помимо R-22: 128 участников foreach, 64 модификатора на одну способность, не более 4096 активных вкладов, 64 жетонов эффектов на существе и 1024 в матче, 256 независимых ContinuousEffectInstance, 128 InteractionRule на одно взаимодействие и 256 ожидающих триггеров. Каждая проверенная вершина выражения, шаг обхода, попытка добора, создание/удаление жетона и его компонента, применение эффекта, проверка InteractionRule и волна checkpoint учитываются в бюджете внешней команды. Превышение — technical_error с диагностикой, а не зависание.
