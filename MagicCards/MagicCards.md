...
**Версия:** 0.11 · **Дата:** 16.09.2026 · **Статус:** исходная спецификация; код ещё не реализован.
...
- Бой сохраняет атаки игрока и назначение блоков; в v0.10 отдельный компонент attack_target разрешает атаковать, например, носителя Силы. Базовое существо его не имеет. Свойства вроде полёта ограничивают блокирование, а не вводят прямую атаку на существо.
...
**Рабочие допущения v0.11:** TypeScript и браузерный стек; ограниченная модель блоков; числа баланса, размеры колод/руки, правила производства/хранения и стартовые карты ниже. Это предложения для старта, а не ранее утверждённые решения. Их изменения оформляются следующей редакцией.
...
Legacy-термин CharacterRef и селекторы *_character означают только «игрок или существо». В редакторе эта область так и подписывается; слово «персонаж» не используется как общее название цели. Тип types:champion («Носитель Силы», прежнее рабочее название «Персонаж Силы») — отдельный тип, не часть этого объединения. Колода не содержит ResourceDefinition, AbilityDefinition, EffectTokenDefinition или InteractionRule как игровых карт: type: resource ссылается на типы ресурса через производство. Произвольный порядок допустимых карт задаёт исходный список для перемешивания, а библиотечные ссылки разрешаются до матча. Ограничение 24/2 с явными исключениями для источников — рабочее правило стартового профиля, проверяемое и редактором, и ядром.
...
Предикат is_damaged имеет единственное каноническое поле object: HealthRef. В фильтре явно указывается {context: candidate}, в условии эффекта — выбранная цель. Старые target и отсутствие object в selector.filter нормализуются только legacy-адаптером; вне фильтра отсутствующий object всегда ошибка. Is_damaged означает здоровье ниже максимального (для permanent — markedDamage > 0); не проверяет броню или Силу.
...
Различать schemaVersion (формат данных), rulesetVersion (семантика игры), packVersion (контент и баланс) и engineVersion (реализация). Для этой редакции нормативны **schemaVersion = 3, rulesetVersion = 0.11**. Это проектные версии: код и машинные схемы ещё не созданы.
...
```json
{
  ...
  "cards": [
    "cards/heated_insight.json"
  ],
  "decks": []
}
```

Requires — множество требований к реализации, а не обязательно минимальный точный список: оно должно покрывать все реально использованные расширяемые узлы. Известные лишние требования допустимы (загрузчик предупреждает, что они сужают совместимость), неизвестные или неподдерживаемые — ошибка даже если не используются. Пропуск необходимой capability — ошибка. В эталонных манифестах приводится минимальный список по разрешённому замыканию AST/refs. Базовые компоненты типов, выбор целей и зоны входят в указанный ruleset. ComponentRegistry хранит capabilityId расширяемого компонента; если он не входит в базовый профиль, набор также объявляет этот capabilityId. Структурное свойство носителя health и требование реализации effect.heal — разные пространства имён.
...
Нормализация выполняется до связывания всего набора. Смешивание typeId с legacy type или несовместимых новых и старых объявлений — ошибка, а не выбор одного значения по приоритету. В манифесте v3 явное разрешение раннего синтаксиса задаётся importOptions: {legacySyntax: true}; по умолчанию false. Включение разрешает только документированные старые формы, не неизвестные узлы. Результат и факт миграции видны в диагностике. Эталонные examples/duality ниже используют канонические карточные typeId и не требуют флага. Отдельные примеры appliesTo/activeOn в 7.17 помечены как legacy и при импорте требуют его.
...
Новое разовое действие **effect.grant_ability** использует тот же план выдачи, что operation: grant_ability внутри continuous. В эффекте target — совместимый AbilityHostRef/PlayerRef или разрешённое множество, ability — ссылка с params либо прежнее встроенное тело, duration — одна из 16.7. Локальный id внутри ability не нужен при временной выдаче: уникальный origin создаёт ядро.
...
На проверке 16.09.2026 Git имеет историю коммитов (HEAD fdd1bbd), текущую main и remote origin. Это снимок окружения, не обязательная ветка разработки. Перед кодом проверить актуальное состояние, настройки и выбрать рабочую ветку. Хранилище Obsidian автоматически не перемещать.
...
|---|---|---|
| E0 | Спецификация 0.11 | Контракты ядра, библиотека способностей, события/фишки/лимит поля/жетоны эффектов/оба режима экономики/составные типы/чары/Сила/перемещения, E8/P1, бот, мастерская, колоды и PR описаны; конвертеры/сеть не расширяют первый выпуск |
...
| T-208 | Requires: нужный, пропущенный, лишний и неизвестный | Пропуск/неподдерживаемый id отклонены; известный лишний допустим с предупреждением; duality не требует неиспользуемый gain_resource. |
...
| T-210 | DurationSpec и is_damaged канонической/старой формы | Объект duration и object предиката работают; старые сокращения доступны только при legacySyntax, неизвестные варианты отклонены. |
...
| T-212 | Канонический стартовый набор без адаптера | Все 9 карт duality и учебное заклинание проходят schemaVersion 3 без importOptions; экспорт не возвращает type/speed. |
...
| D-45 | Уточнение по ревью v0.11 | Canonical DurationSpec/object/typeId и on_enter; legacySyntax включается явно. Requires покрывает используемые возможности, известные лишние допустимы с предупреждением. Лимиты считаются на behavior и definition после разрешения AST-ссылок. |
...
**Единый DurationSpec.** В каноническом JSON поле duration всегда объект: {type: "until_end_of_turn"}, {type: "permanent_on_instance"} либо {type: "until_source_leaves"}. Имя варианта в таблице/прозе — сокращённое описание, не строковый JSON. Строковая форма допустима только при явном legacySyntax и нормализуется в объект до проверки. Until_next_controller_turn_start в 7.14 — будущий вариант статусов, а не поддержка этой механики в текущем ядре. Допустимые варианты дополнительно ограничены операцией: create_continuous_effect поддерживает только until_end_of_turn; EffectTokenInstance — сроки, перечисленные в 7.19. Наличие общей формы duration не открывает все варианты каждой операции.
